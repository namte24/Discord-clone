  ...
  return (
    <div className="flex items-center p-1 pl-5 my-5 mr-2 hover:bg-discord_messageBg group">
      ...
      <div className="flex flex-col">
        ...
        <p className="text-sm text-discord_userSectionHover">{message}</p>
      </div>
      {user?.email === email && (
        <div
          className="hover:bg-discord_deleteIconBg p-1 ml-auto rounded-sm text-discord_deleteIconBg hover:text-white cursor-pointer"
          onClick={() =>
            db
              .collection("channels")
              .doc(channelId)
